...
mice_data_n <- NormalizeData(mice_data, normalization.method = "LogNormalize")
...
library(uwot)
library(Rtsne)
pc_var <- mice_data@reductions$pca_sdev^2
pc_mat <- mice_data@reductions$pca@cell.embeddings[,1:50]
pc_mean <- colMeans(pc_mat)
pc_dev <- sweep(pc_mat, 2, pc_mean, "-")
pc_var <- apply(pc_dev, 2, var)
pc_sd <- sqrt(pc_var)
var_exp <- pc_var / sum(pc_var)
sum(var_exp)
library(ggplot2)

df <- data.frame(PC = 1:length(var_exp), Variance = var_exp)
scree_1_plot <- ggplot(df, aes(x = PC, y = Variance)) + 
...
library(ggplot2)
scree_plot <- ggplot(data.frame(x=1:length(var_exp), y=cumulative_var_exp), aes(x=x, y=y)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(name="Principal Component") +
  scale_y_continuous(name="Cumulative Variance Explained") +
  theme_minimal()
scree_plot + scree_1_plot
